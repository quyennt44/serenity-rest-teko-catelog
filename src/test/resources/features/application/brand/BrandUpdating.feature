@BrandUpdatingSpecification
Feature: Updating existing brands

  Background: 
    Given User has access to application as 'user1'
    When User creates a brand

  Scenario Outline: TC01 Check Fields With Null Value
    And User set value for field '<field_name>' to 'null'
    And User send request to 'update' brand
  ...
  #Scenario Outline: TC02 Check String Fields With Empty Value
    #And User set value for field '<field_name>' to 'empty'
...
#
  #Scenario Outline: TC03 Check String Fields With Spaces Value Only
    #And User set value for field '<field_name>' to 'spaces_only'
    #And User send request to create brand
    #Then The response shows hppt code as <http_code>
    #And The response shows content as '<response_content>'
    #And The data in database should be checked: '<check_database>'
...
#
  #Scenario Outline: TC04 Check String Fields Containing Spaces Value
    #And User set value for field '<field_name>' to 'leading_ending_spaces' in length <length>
    #And User send request to create brand
    #Then The response shows hppt code as <http_code>
    #And The response shows content as '<response_content>'
    #And The data in database should be checked: '<check_database>'
...
#
    #Examples: 
      ...
      #| name       |     10 |       200 | success          | Yes            |
      #| code       |     10 |       200 | success          | Yes            |
#
  #Scenario Outline: TC05 Check Fields With Max Length
    #And User set value for field '<field_name>' to 'random_string' in length <length>
    #And User send request to create brand
    #Then The response shows hppt code as <http_code>
    #And The response shows content as '<response_content>'
    #And The data in database should be checked: '<check_database>'
...
#
  #Scenario Outline: TC07 Check Fields With Special Characters
    #And User set value for field '<field_name>' to 'contain_special_characters' in length <length>
    #And User send request to create brand
    #Then The response shows hppt code as <http_code>
    #And The response shows content as '<response_content>'
    #And The data in database should be checked: '<check_database>'
...
#
    #Examples: 
      #| field_name | length | http_code | response_content | check_database |
      #| name       |     10 |       200 | success          | No             |
      #| code       |     10 |       400 | INVALID          | No             |
#
  #Scenario Outline: TC072 Check Fields Containing Space Characters
    #And User set value for field '<field_name>' to 'contain_space_characters' in length <length>
    #And User send request to create brand
    #Then The response shows hppt code as <http_code>
    #And The response shows content as '<response_content>'
    #And The data in database should be checked: '<check_database>'
